...
if [ "${EXPORTMODE}" == "VERIFY" ]; then
    ...
    su -c "${CONDAEVAL};"'ls -alh /opt/conda/pubenvs/;df -h;conda create --clone "$SRCENVNAME" -p /opt/conda/pubenvs/"${DSTENVNAME}";' $NB_USER
    ...
    su -c "${CONDAEVAL};conda activate ${SRCENVNAME};LOGFILE="${LOGFILE}" python /opt/condaenv-adapter.py "'${WETRUNARGS};' $NB_USER
else
    echo 
fi
